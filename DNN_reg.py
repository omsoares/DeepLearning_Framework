...
class DNN_reg:
    """
    This is class is to be used for generating deep learning models for regression problems.
    ...
    def print_fit_results(self, train_scores, val_scores):
        ...
        :return: None
        """
        print('val_r2: ', val_scores[1])
        print('val_loss: ', val_scores[0])
        print('train_r2: ', train_scores[1])
    ...
    def find_best_model(self):
        ...
        if self.model_selection_history:
            ...
            for dic in self.model_selection_history:
                if dic['mean_val_r2'] > max_val_r2:
                    best_model = dic
                    max_val_r2 = dic['mean_val_r2']
        # If all models have score 0 assume the first one
        if best_model is None:
            best_model = self.model_selection_history[0]
        # print("Best model:")
        # self.print_parameter_values()
        # print("R2: " + str(max_val_r2))
        return best_model, max_val_r2
    ...
    def write_model_selection_results(self, root_dir, file_name):
        ...
        sequence = ['mean_val_r2', 'mean_val_loss', 'mean_train_r2',
        ...
        df = pd.DataFrame(d, columns=sequence).sort_values(['mean_val_r2'], ascending=[False])
        if not os.path.exists(root_dir):
            os.makedirs(root_dir)
        i = 0
    ...
    def save_best_model(self):
        ...
        i = 0
        while os.path.exists("best_DNN_models/" + "DNN_reg" + str(i) + '.json'):
            i += 1
        root = "best_DNN_models"
        if not os.path.exists(root):
            os.makedirs(root)
        file_name_model = os.path.join(root, "DNN_reg" + str(i) + '.json')
        file_name_weights = os.path.join(root, "DNN_reg" + str(i) + '.h5')
    ...
    def multi_model_selection(self, root_dir, experiment_designation, n_iter=10, cv=5):
        ...
        self.create_DNN_model()
        # X_train, X_valid, y_train, y_valid = train_test_split(self.X_train, self.y_train, test_size=0.25)

...
class DNN_MT_bin(DNN_bin):
    ...
    def generate_y_dicts(self):
        if self.splitted == True:
            ...
            self.y_s = {}
            for output,y in self.output_list,self.y_list:
                self.y_s[output] = y
    ...
    def create_DNN_model(self, print_model=True):
        ...
        fundamental_parameters = ['dropout', 'output_activation', 'optimization', 'learning_rate',
                                  ...
                                  'units_in_hidden_layers', 'nb_epoch', 'batch_size','types']
        ...
        # Input layer
        x = Input(shape=(self.parameters['units_in_input_layer'],),name = "inputs")
        ...
        self.metric_types = {"bin":"accuracy","multi": "accuracy","reg":"mse"}
        self.metrics_dict = {}
        self.loss_dict = {}

        for output,type in output_list,self.parameters['types']:
        ...
        model = Model(input=[inputs],outputs = list(self.output_dict.keys))
        model.compile(optimizer=optim, loss = self.loss_dict,metrics = self.metrics_dict)
